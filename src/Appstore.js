...
const Appstore=()=>{
    ...
    const getSearchResults=()=>{
        const searchResults=appsList.filter(eachItem=>eachItem.appName.toLowerCase().includes(search.toLowerCase())

        )
        return searchResults
    }
    const getActiveTabApps=(searchResults)=>{
        const filteredApps=searchResults.filter(eachSearchedApp=>eachSearchedApp.category=== activeId)
        return filteredApps
        ...
        <div className="app-container">
            <div className="app-store">
                ...
                <div className="search-container">
                    <input type="search" className="user-input" onChange={onChangeSearchInput}/>
                    <img src={SEARCH_ICON_URL} alt="search" className="search-icon"/>

                </div>
                <ul className="tabs-list">
                    {tabsList.map(eachTab=>(
                        <TabItem  key={eachTab.tabId} tabDetails={eachTab} setActiveTabId={onSetActiveTabId} isActive={activeId===eachTab.tabId}/>
                    ))}
